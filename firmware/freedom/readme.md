...
Enter the bootloader *without* clearing the EEPROM in the following ways (useful for preserving settings while upgrading firmware):
...
* **Serial terminal configurator**: Connect to keyboard through serial interface, then select bootloader in the menu. 
...
If the firmware gets so corrupt that the MCU doesn't get to the point where it can enter the bootloader via QMK, first try the `physical reset button` method. If that doesn't work, it must be cleared manually.
