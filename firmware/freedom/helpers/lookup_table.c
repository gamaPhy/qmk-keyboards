...
float ratio(int sensor_max, int sensor_min, int base_val) {
  return ((float)sensor_max - (float)base_val) /
         ((float)sensor_min - (float)base_val);
}

float compute_b_param(int sensor_max, int sensor_min, int base_val) {
  return (float)((X_MAX_mm * cbrt(ratio(sensor_max, sensor_min, base_val)) -
                  X_MIN_mm) /
                 (1.0 - cbrt(ratio(sensor_max, sensor_min, base_val))));
}

float compute_a_param(float b_param, int sensor_max, int sensor_min,
                      int base_val) {
  return ((float)sensor_min - (float)base_val) *
         (float)pow(b_param + X_MIN_mm, 3);
}
...
float int_components_to_float(int val, int fractional_component_as_int) {
  float fractional_component_as_float =
      (float)fractional_component_as_int / (float)INT_MAX;
...
int determine_sensor_base_offset(kb_config_t *kb_config) {
  ...
  // TODO: change this to return based on the preprocessor configs passed in
  return DRV5056A3_BASE_OFFSET;
}
...
// Computes and stores the `a` and `b` parameters of the best-fit scaling
// equation.
  ...
  for (int row = 0; row < MATRIX_ROWS; row++) {
    for (int col = 0; col < MATRIX_COLS; col++) {
      if (pin_scan_modes[row][col] == ANALOG) {
        ...
        kb_config->matrix_scaling_params[row][col].b_fractional_component =
            fractional_component_as_int(b_param);
        kb_config->matrix_scaling_params[row][col].a =
            compute_a_param(b_param, max, min, base_val);
    ...
    for (int row = 0; row < MATRIX_ROWS; row++) {
      for (int col = 0; col < MATRIX_COLS; col++) {
        if (pin_scan_modes[row][col] == ANALOG) {
          ...
          float b = int_components_to_float(
              kb_config->matrix_scaling_params[row][col].b,
              kb_config->matrix_scaling_params[row][col]
                  .b_fractional_component);
          float base =
              (float)kb_config->matrix_scaling_params[row][col].base_value;
          for (int adc_val = 0; adc_val < MAX_ADC_READING; adc_val++) {
            float val_mm = (cbrt(a / ((float)adc_val - base)) - b);
            int sensor = sensor_nums[row][col];

            if (val_mm < X_MIN_mm) {
              sensor_lookup_table[sensor][adc_val] =
                  mm_to_lookup_table_val(X_MIN_mm);
            } else if (mm_to_lookup_table_val(val_mm) >
                       KEY_MAX_dmm * LOOKUP_TABLE_MULTIPLIER) {
              sensor_lookup_table[sensor][adc_val] =
                  KEY_MAX_dmm * LOOKUP_TABLE_MULTIPLIER;
            } else {
              sensor_lookup_table[sensor][adc_val] =
                  mm_to_lookup_table_val(val_mm);
            }
          }
        }
      }
